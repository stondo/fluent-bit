                           ...
                           const char *k, const char *v)
{
    ...
    if (prop_key_check("tag", k, len) == 0 && tmp) {
        ...
        ins->rate_gate_max_records = (size_t) ret;
    }
        ...
        ins->rate_gate_resume_ratio = atof(tmp);
        flb_sds_destroy(tmp);

        if (ins->rate_gate_resume_ratio <= 0.0 ||
            ins->rate_gate_resume_ratio >= 1.0) {
...
static int flb_input_rate_gate_is_limited(struct flb_input_instance *ins)
{
    ...
    if (ins->rate_gate_max_records > 0) {
        ...
        /* Check the last calculated rate from the previous window */
        if (ins->rate_records > effective_max_records) {
            return FLB_TRUE;
        }
        /* * ALSO check the current accumulated count in the ongoing window.
         * This ensures the gate triggers even before the first window expires.
         */
    ...
    if (ins->rate_gate_max_bytes > 0) {
        ...
        /* Check the last calculated rate from the previous window */
        if (ins->rate_bytes > effective_max_bytes) {
            return FLB_TRUE;
        }
        /* Check the current accumulated bytes in the ongoing window */
...
static int flb_input_rate_gate_can_resume(struct flb_input_instance *ins)
{
    ...
    if (ins->rate_gate_max_records > 0) {
        ...
        if (ins->rate_records > resume_max_records) {
            return FLB_FALSE;
        }
    ...
    if (ins->rate_gate_max_bytes > 0) {
        ...
        if (ins->rate_bytes > resume_max_bytes) {
            return FLB_FALSE;
        }
...
static void flb_input_rate_gate_collect_backpressure(struct flb_input_instance *ins)
{
    ...
    mk_list_foreach(head, &ins->tasks) {
        ...
        mk_list_foreach(retry_head, &task->retries) {
            ...
            ins->rate_gate_retry_attempts += retry->attempts;
        }
    }
}
...
void flb_input_rate_gate_maybe_resume(struct flb_input_instance *ins)
{
    ...
    if (can_resume == FLB_TRUE &&
        ...
        if (ins->p->cb_resume) {
            ...
            flb_info("[input] %s resume (rate gate)",
                     flb_input_name(ins));
        }
                           ...
                           size_t bytes)
{
    ...
    if (timestamp < ins->rate_window_start ||
        (timestamp - ins->rate_window_start) < rate_window_size) {
        return;
    }

    elapsed_seconds = (double) (timestamp - ins->rate_window_start) /
                      (double) FLB_NSEC_IN_SEC;
    if (elapsed_seconds <= 0.0) {
        return;
    }

    ins->rate_records = (double) ins->rate_window_records / elapsed_seconds;
    ins->rate_bytes = (double) ins->rate_window_bytes / elapsed_seconds;
